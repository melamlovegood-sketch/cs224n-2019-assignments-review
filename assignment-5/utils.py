...
def pad_sents_char(sents, char_pad_token):
    ...
    max_sentence_length = max((len(sentence) for sentence in sents), default=0)
    ...
    for sentence in sents:
        ...
        for word in sentence:
            ...
            padded_word = truncated_word + [char_pad_token] * (
                max_word_length - len(truncated_word)
            )
            padded_sentence.append(padded_word)
        padded_sentence.extend(
            [padding_word[:] for _ in range(max_sentence_length - len(sentence))]
        )
